...
function marines_suppress_cult_influence() {
    with (obj_star) {
        for (var p = 1; p <= planets; p++) {
            ...
            // Reduce Tyranid influence directly (a pure decrease needs no total-renormalisation).
            p_influence[p][eFACTION.TYRANIDS] -= min(_reduction, p_influence[p][eFACTION.TYRANIDS]);
