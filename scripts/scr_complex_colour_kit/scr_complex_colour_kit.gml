...
function setup_complex_livery_shader(setup_role, unit = "none") {
    ...
    if (_in_creation) {
        ...
        if (_is_unit) {
            ...
            // Institution groups (>10) have no livery slot of their own; render them with the
            // HQ (company 0) livery so their specialists match the chapter command colours.
            var _livery_company = (unit.company > LINE_COMPANY_MAX) ? 0 : unit.company;
            var _company_livery = obj_ini.company_liveries[_livery_company];
